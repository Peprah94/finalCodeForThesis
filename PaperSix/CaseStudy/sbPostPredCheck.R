...
load("CaseStudy/solitarybees/estimateCaseStudy.RData")
idm <- estimatesCaseStudy
rm("estimatesCaseStudy")
load("CaseStudy/solitarybees/estimateCaseStudyIG.RData")
...
rm("idm")
#select bumblebees
simulations_all <- simulations_all[[3]]
...
# clusterExport(cl, c("runFunction", "parallelRun"))

# bootstrapEstimates <- pbapply::pblapply(simData[1:4], function(x){
