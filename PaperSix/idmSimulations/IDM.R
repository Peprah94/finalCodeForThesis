...
#install.packages(c("pbapply", "nimble", "MCMCglmm", "coda", "MCMCpack", "boot", "MASS", "parallel","ggmcmc"))


#install.packages("do.parallel")
for(iCount in 1:2){
  library(doParallel)
  library(foreach)
  library(parallel)
  ...
  sim <- simulations_all_na[126:130]
  ...
  rep_estimates <- foreach(iter = seq_along(sim), .packages = c("pbapply", "nimble", "MCMCglmm", "coda", "MCMCpack", "boot", "MASS", "parallel","ggmcmc")) %dopar% {
    tryCatch({ run_nimble_model(sim[[iter]], model = "IDM",covariance_prior = "LV", method = modelVars[iCount]) }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
  }
  ...
  save(rep_estimates, file=paste0("idmSimulations/IDM/estimateDataIDM",modelVars[iCount],"SimCov5.RData"))
  stopCluster(cl)
}
