...
rep_estimates <- foreach(iter = seq_along(sim), .packages = c("pbapply", "nimble", "MCMCglmm", "coda", "MCMCpack", "boot", "MASS", "parallel","ggmcmc")) %dopar% {
  tryCatch({ run_nimble_model(sim[[iter]], model = "Spe",covariance_prior = "LV", method = modelVars) }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")})
}
...
save(rep_estimates, file=paste0("idmSimulations/Species/estimateDataSpecies",modelVars,"SimCov5.RData"))
stopCluster(cl)
#}
