...
for(j in 1:p_sampling){
  #sampling_form[j] <- paste0("-1*sampling_covs.im[[",j,"]]")
...
x0 <- sampling_covs.im[[1]]$xcol
y0 <- sampling_covs.im[[1]]$yrow
...
df.sp2 <- SpatialPointsDataFrame(coords = gridlocs@coords,
                                 data = data.frame(w=c(log(Lam$v)),
                                                   ew=c(Lam$v)))

df.sp2$retprob <- psych::logistic(df.sp2$w)
r <- raster::raster(df.sp2)
...
for(i in 1:nspecies){
  retprobslik <- raster::extract(prob.rast,cbind( environment_list$Eco_PP[[i]]$x, environment_list$Eco_PP[[i]]$y))
