...
cov4.rast <- raster(simulateddata[[iter]]$cov[[4]])
crs(cov4.rast) <- BNGproj
cov4.spix <- as(cov4.rast,"SpatialPixelsDataFrame")
...
#                              # PC-prior on range: P(practic.range < 0.05) = 0.01
#                              prior.range = c(input$sampling$hyperparameters$range, 0.1),
#                              # PC-prior on sigma: P(sigma > 1) = 0.01
#                              prior.sigma = c(sqrt(input$sampling$hyperparameters$sigma2), 0.1))
...
save(rr, file = paste0("VSEDetect/VSEDetect",iter,".RData"))
}
