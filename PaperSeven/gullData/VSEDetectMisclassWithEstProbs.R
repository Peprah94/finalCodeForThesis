...
estProbs <- lapply(as.list(1:30), function(i){
  load(paste0("VSEDetectMisclassProbs/VSEDetectMisclass", i,".RData"))
  VSEDetectMisclass <- matrix(mcmc.out$summary[1:4, 1], 2,2, byrow = FALSE)
})
...
for(iter in 1:30){
  ...
  for(i in 1: nspecies){
    spdes[[i]] <- inla.spde2.pcmatern(mesh = mesh,
                                      ...
                                      prior.range = c(input$ecological$hyperparameters$range[i], 0.01),
                                      # PC-prior on sigma: P(sigma > 1) = 0.01
                                      prior.sigma = c(sqrt(input$ecological$hyperparameters$sigma2[i]), 0.01))
  }
  ...
  class_prob <- estProbs[[iter]]
  rr <- est_par(p11 = class_prob[1,1], p22 = class_prob[2,2],
                model = "VSEDetect",
                return = "noinla")
  
  save(rr, file = paste0("VSEDetectMisclass/VSEDetectMisclass",iter,".RData"))
}
