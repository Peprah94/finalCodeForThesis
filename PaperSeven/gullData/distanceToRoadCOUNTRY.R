...
gbif_data <- allDataForModels$spPointsGbif@data
bioclim = FALSE
  message("Getting the countries from the GBIF data")
  countries <- unique(gbif_data$COUNTRY)
  if(is.null(countries)){countries <- unique(gbif_data$COUNTRY)}
  ...
  for(country.tag in 1: length(countries)){
    ...
    all_data <- gbif_data%>%
      dplyr::filter(COUNTRY== countries[country.tag])
      #dplyr::filter(countryCode == countries[country.tag])
    #States <- geodata::gadm(country = countries[country.tag], level = 2, path=getwd())
    States <- raster::getData("GADM", country = countries[country.tag], level = 2)
    
    States <- States%>%
      sf::st_as_sf()%>%
      terra::vect(.)%>%
      terra::project(., "+proj=robin +datum=WGS84 +units=km")%>%
      sf::st_as_sf()%>%
      as(., "Spatial" )
    ...
    adminCoords <- df_data@coords

    if(bioclim == TRUE){
      altitude <- raster::extract(alt, df_data, df=T)
      bio_data <- raster::extract(bio, df_data, df = T)
      data_df[[country.tag]] <- cbind(data_with_admin, altitude, bio_data,adminCoords)%>%
        data.frame()
    }else{
      data_df[[country.tag]] <- cbind(data_with_admin, adminCoords)%>%
        data.frame()
    }
  }
  ...
  distance_estimate <- function(i, data){
    ...
    dataFiltered <- data%>%
      filter(NAME_2 == states[i])
    bb_box <- sp::bbox(cbind(lon = dataFiltered$decimalLongitude, lat = dataFiltered$decimalLatitude))
    bg_map <- ggmap::get_map(bb_box,
                             maptype = "toner-lite",
                             source = "stamen",
                             color="bw",
                             force=T)
  ...
  dataWithDistance1 <- lapply(as.list(seq(1,30)), function(x){
